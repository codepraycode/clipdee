...
// Read clipboard (Chrome version)
async function readClipboard() {
    try {
        // Chrome requires user gesture for clipboard access
        const text = await navigator.clipboard.readText();
        return text.trim();
    } catch (err) {
        console.error('Failed to read clipboard:', err);
        throw new Error('Cannot access clipboard. Please allow clipboard permissions when prompted.');
    }
}

// Check if clipboard contains reasonable text worth searching
function isReasonableText(text) {
    ...
    // Skip if it looks like code (has brackets, semicolons, etc.)
    ...
    // Skip if it's too long (probably not a word lookup)
    if (text.length > 200) return false;

    // Skip if it has too many words (probably copied paragraph/article)
...
function extractWord(text) {
    ...
    // First check if the text is reasonable to search
...
async function getDefinition() {
    ...
    try {
        ...
        if (!word) {
            showError('No searchable word found in clipboard');
            return;
        }
